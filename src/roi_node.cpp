...
#include "robot_nodes/message.h"            //Our global message message format
...
void image_callback(const robot_nodes::message &recvMsg)
{
    try
    {
        ...
        robot_nodes::message sendMsg;    //message we are about to publish format;
...
int main(int argc, char **argv)
{
    ...
    publisher = publishNode.advertise<robot_nodes::message>("roi_topic", 1) ;                         //publisher
    ros::Subscriber subscriber = subscribeNode.subscribe("camera_imu_topic", 1, image_callback);   //subscriber
